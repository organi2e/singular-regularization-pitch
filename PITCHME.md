...
## 自己相関行列$W^TW$の固有値は$1$に近いべき
 - 各層の行列積の入出力スケールは等しいべき
 - 特にRNNでは出力が再起するのでスケールの影響を強く受ける
   - 大きい固有値に対応したベクトルが特に強調
   - 活性化関数の形が線形なほど発散する
 - $W^TW$の固有値 $=$ $W$の特異値の二乗
...
$V$も直交行列なので  
$W^TW=V^T\Sigma^2V=E$になる条件は$\Sigma=E$  

---  

つまり，$W^TW$を単位行列に近づけること  
= **$W$の特異値を全て** $1$に正則化すること

---    

## 本発表の主張
### $W^TW$の特異値が$1$になるように正則化を行うべき
(本発表では **特異値正則化と仮称**)
...
## 正則化項の設計(2種類)
  ...
  2. $\rm{tr}\left(\Sigma-\log\Sigma-E\right)\Leftrightarrow\sum_k\left(\sigma_k-\log\sigma_k-1\right)$
   - $U\Sigma V^T=W$, $\sigma_k=\diag\rm{diag}_k\left(\Sigma\right)$
   - 演算量は多いが数値的に安定
   - 本発表ではこちらを採用
...
## 結論と展望
 ...
 - 自己相関行列を単位行列にするか特異値を$1$にする正則化手法を提案
  - 目的とする挙動は等価
 - DCGANの数値実験ではmode collapseを防げるなど有意な結果が得られた
  - モデルの層が浅くL2正則化との差が体現しづらかったため，より深いモデルでの評価が必要